...
class PaymentController extends Controller
{
    public function success(Request $request)
    {
        ...
        $mrh_pass1 = $is_test ? env('ROBO_TEST_PASS1') : env('ROBO_PAY_PASS1');
    ...
    public function fail(Request $request)
    {
        ...
        $my_crc = strtoupper(md5("Marellakz:$out_summ:$inv_id:$mrh_pass1"));
        $robo_string = "https://auth.robokassa.kz/Merchant/Index.aspx?MerchantLogin=Marellakz&Culture=ru&Description=Оплата заказа № {$order->order_number} в интернет магазине&Encoding=utf-8&OutSum={$out_summ}&InvId={$inv_id}&SignatureValue={$my_crc}&IsTest={$is_test}";
        $data = array(
